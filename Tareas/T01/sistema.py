...
class Sistema:
    def __init__(self, lista_usuarios, lista_mercados, lista_orders,
                 lista_monedas, lista_match):
    ...
    def start(self):
        ...
        while True:
            ...
            if opcion == "8":
                ...
                while True:
                    mercado_a_registrar = input("Ingrese el ticker del mercado"
                                                " al que se quiere registrar:"
                                                " ")
                    for mercado in self.lista_mercados:
                        if (mercado_a_registrar == mercado.ticker and
                                    mercado not in usuario_actual.mercados):
                ...
                if opcion == "1":
                    ...
                    while True:  # manejo de errores
                        mercado_elegido = input("Ingrese el ticker del "
                                                "mercado especifico: ")
                        for mercado in usuario_actual.mercados:
                            if mercado.ticker == mercado_elegido:
                                mercado_elegido = mercado
                                break
                        if isinstance(mercado_elegido, Mercado):
                            break  # manejo de errores
                        else:
                            print("Mercado invalido. Recuerda que debes"
                                  " estar registrado en el mercado.")
                    mercado_elegido.desplegar_informacion()
            elif opcion == "5":
                print("Los siguientes mercados tienen orders activas. "
                      "Puede que hayan algunas que no puedas ver "
                      "en el caso de que no seas Investor.")

                for mercado in usuario_actual.mercados:
                    if (mercado.asks_activos != [] or
                                mercado.bids_activos != []):
                        print("MERCADO:", mercado.ticker)
                    for order in mercado.asks_activos:
                        mostrar = self.fecha_limite(order.tiempo,
                                                    usuario_actual)
                        if mostrar:
                            diccionario = {"username" : usuario_actual.username
                                           ...
                                           "Hora"     : strftime("%H:%M")}
                            self.consulta_orders_activas.append(diccionario)
                            diccionario = {"tipo"   : "ask",
                                           "amount" : order.divisa_venta}
                            self.consulta_orders_activas[-1].update(
                                diccionario)
                            print(order)

                    for order in mercado.bids_activos:
                        mostrar = self.fecha_limite(order.tiempo, usuario_actual)
                        if mostrar:
                            diccionario = {"username": usuario_actual.username
                                           , "order_id": order.id,
                                           "price": order.moneda_de_cambio,
                ...
                while True:  # manejo de errores
                    opcion_consulta = input("Ingrese la opcion de la"
                                            " consulta a realizar: ")
                    if (opcion_consulta != "1" and opcion_consulta != "2" and
                                opcion_consulta != "3"):
                ...
                if opcion_consulta == "1":
                    ...
                    for moneda in self.lista_monedas:
                        print(moneda)
                    while True:  # manejo de errores
                        moneda_elegida = input("Ingresa uno de los simbolos "
                                               "de la lista anterior de "
                                               "monedas: ")
                        monedas = self.lista_monedas  # aux para que entre
                        monedas_con_mismo_simbolo = list(filter(lambda x:
                                                                x.simbolo ==
                                                                moneda_elegida,
                                                                monedas))
                        # verificar si hay monedas igual a la elegida
                        if len(monedas_con_mismo_simbolo) > 0:
                    ...
                    for mercado in self.lista_mercados:
                        if (len(mercado.asks_activos) != 0 or
                                    len(mercado.bids_activos) != 0):
                            if (mercado.divisa_compraventa == moneda_elegida or
                                        mercado.moneda_de_cambio ==
                                        moneda_elegida):
                                mercados_activos += 1
                                mercados_con_la_moneda.append(mercado)
                    print("Hay " + str(mercados_activos) +
                          " mercados con orders activas de", moneda_elegida)
                    for mercado in mercados_con_la_moneda:
                        ...
                        orders_mercado = sorted(mercado.orders,
                                                key=lambda elem:
                                                (int(elem.año), int(elem.mes),
                                                 int(elem.dia)),
                                                reverse=True)
                        print("-Ultima order: ", orders_mercado[0])
                        if match_en_mercado != []:
                            match_en_mercado.sort(key=lambda elem:
                            (int(elem.año),int(elem.mes), int(elem.dia)),
                                                  reverse=True)
                            ultimo_match = match_en_mercado[0]
                            print("-Precio actual de",
                                  mercado.divisa_compraventa, ":",
                                  ultimo_match.bid.moneda_de_cambio,
                                  mercado.moneda_de_cambio)
                ...
                for symbol, saldo in usuario_actual.balance_currencies.items():
                    print(symbol, saldo)
                    for mercado in self.lista_mercados:
                        if mercado.ticker == (symbol + "DCC"):
                            mercado_analizado = mercado
                            break
                        else:
                            mercado_analizado = ""
                    match_en_mercado = []
                    for match in self.lista_match:
                        if match.ask.mercado == mercado_analizado:
                            match_en_mercado.append(match)
                    if len(match_en_mercado) > 0:
                        match_en_mercado = sorted(match_en_mercado,
                                                  key=lambda x: x.tiempo,
                                                  reverse=True)
                        precio = d(match_en_mercado[0].ask.moneda_de_cambio)
                        # aux por 79 caracteres
                        saldo_en_dcc += (precio * saldo)
                    else:
                        saldo_en_dcc += saldo
                print("Todas sus currencies en valor DCC equivalen a :",
                      str(saldo_en_dcc))
            elif opcion == "10":
                print("1: Desplegar orders en una fecha especifica\n2: "
                      "Desplegar orders entre 2 fechas\n"
                      "3: Desplegar orders en un mercado especifico")
                while True:
                    opcion_desplegar_orders = input("Ingrese una de las "
                                                    "opciones (1, 2 o 3): ")
                    if opcion_desplegar_orders != "1" and \
                                    opcion_desplegar_orders != "2" and \
                                    opcion_desplegar_orders != "3":
                        print("Opcion invalida")  # manejo de errores
                    else:
                        break
                if opcion_desplegar_orders == "1":
                    valido = False
                    while not valido:
                        fecha1 = input("Ingrese la fecha especifica"
                                       " (YYYY-MM-DD): ")
                        valido = self.fecha_limite(fecha1, usuario_actual)
                        if not valido:
                            print("No puedes ver registro de orders antes "
                                  "de los últimos 7 días. "
                                  "Para hacerlo debes ser Investor")
                    fecha2 = ""
                    impresiones = []
                    for order in self.lista_orders:
                        if order.mercado in usuario_actual.mercados:
                            imprimio = order.desplegar_order_por_fecha(fecha1,
                                                                       fecha2)
                    ...
                    while not valido1 or not valido2:
                        ...
                        if not valido1 or not valido2:
                            print("No cumple los requisitos. Ingresa otra "
                                  "vez.")
                    impresiones = []
                    # ordenadas
                    orders_por_fecha = sorted(self.lista_orders,
                                              key=lambda elem: elem.tiempo)
                    for order in orders_por_fecha:
                        if order.mercado in usuario_actual.mercados:
                            imprimio = order.desplegar_order_por_fecha(fecha1,
                                                                       fecha2)
                            impresiones.append(imprimio)
                    ...
                    while not volver:
                        while True:
                            mercado_a_desplegar = input("Ingrese el ticker del"
                                                        " mercado a observar o"
                                                        " 0 para volver "
                                                        "al menu: ")
                            for mercado in usuario_actual.mercados:
                                if mercado_a_desplegar == mercado.ticker:
                                    ...
                                    break
                            if isinstance(mercado_a_desplegar, Mercado):
                                break
                            elif mercado_a_desplegar == "0":
                                volver = True
                                break
                        if volver:
                            break
                        orders_por_fecha = sorted(self.lista_orders,
                                                  key=lambda elem:
                                                  elem.tiempo)  # ordenadas
                        impresiones = []
                        for order in orders_por_fecha:
                            valido = self.fecha_limite(order.tiempo,
                                                       usuario_actual)
                            if valido:
                                imprimio = order.desplegar_order_por_mercado\
                                    (mercado_a_desplegar)
                                if imprimio:
                                    self.consulta_orders_historicas.append\
                                    ...
                                    if isinstance(order, Ask):
                                        self.consulta_orders_historicas[-1].\
                                            update({"tipo": "ask",
                                                    "amount":
                                                        order.divisa_venta})
                ...
                while True:
                    usuario_destino = input("Ingresa el nombre de usuario "
                                            "del destinatario: ")
                    for usuario in self.lista_usuarios:
                        if usuario.username == usuario_destino and \
                                        usuario.username != \
                ...
                while cantidad_invalido or simbolo_invalido:
                    monto = input("Ingresa el monto a transferir "
                                  "(ej.: 100 DCC): ")
                    numero = True
                    cantidad = ""
                    simbolo = ""
                ...
                while True:  # se hace este while para poder usar el
                    ...
                    if conteo_orders_dia >= 15:  # limite de orders para Trade
                    ...
                    if conteo_orders_activas >= 5:
                        if not isinstance(usuario_actual, Investor):
                            ...
                            break
                    while True:
                        divisa_compra = input("Ingrese la cantidad de " +
                                              mercado_actual.ticker[0:3] +
                                              " que quieres comprar (0 para "
                                              "volver al menu): ")
                        moneda_de_cambio = input("Ingrese el precio en "
                                                 + mercado_actual.ticker[3:] +
                        ...
                        try:
                            ...
                            print("Formato incorrecto de ingreso. Recuerda "
                                  "solo ingresar el numero de lo pedido.")
                        else:
                            pago = d(divisa_compra) * d(moneda_de_cambio)
                            balance_real = usuario_actual.balance_currencies[
                                mercado_actual.moneda_de_cambio]
                            # balance real va a considerar las
                            # orders activas del usuario en el mercado,
                            # para que
                            # no ingrese mas dinero del que puede pagar
                            for order in self.lista_orders:
                                if (order.mercado.divisa_compraventa ==
                                        mercado_actual.divisa_compraventa and
                                            order.ejecutada == False and
                                            order.usuario == usuario_actual):
                                    if isinstance(order, Ask):
                                        balance_real -= d(order.divisa_venta)
                                    elif isinstance(order, Bid):
                                        balance_real -= d(order.divisa_compra)\
                                                        * d(order.
                                                            moneda_de_cambio)
                                elif (order.mercado.moneda_de_cambio ==
                                          mercado_actual.moneda_de_cambio and
                                              order.ejecutada == False and
                                              order.usuario == usuario_actual):
                                    if isinstance(order, Ask):
                                        balance_real -= d(order.divisa_venta)
                                        ...
                                        balance_real -= d(order.divisa_compra) \
                                                        * d(order.
                                                            moneda_de_cambio)
                            print(balance_real)
                            if pago > usuario_actual.balance_currencies[
                                mercado_actual.ticker[3:]]:
                                print("No tienes suficiente dinero para "
                                      "hacer esa order")
                            elif pago > balance_real:
                                print("No puedes hacer la order porque tienes"
                                      " mas dinero ingresado en otras.")
                    ...
                    if volver:
                        break
                    nuevo_bid = usuario_actual.ingresar_bid(usuario_actual,
                                                            mercado_actual,
                                                            divisa_compra,
                                                            moneda_de_cambio)
                ...
                if cantidad_match_antes < cantidad_match_despues:
                    print("Felicidades! Tu order ha hecho match! Se ha "
                          "actualizado el saldo con la nueva informacion.")
            elif opcion == "13":
                volver = False  # para volver al menu
                cantidad_match_antes = len(self.lista_match)
                while True:
                    ...
                    if conteo_orders_activas >= 5:
                        if not isinstance(usuario_actual, Investor):
                            ...
                            break
                    while True:  # manejo de errores
                        divisa_venta = input("Ingrese la cantidad de " +
                                             mercado_actual.ticker[0:3] +
                                             " que quieres vender (0 para "
                                             "volver al menu): ")
                        moneda_de_cambio = input("Ingrese el precio en " +
                                                 mercado_actual.ticker[3:] +
                                                 " al que vendes una unidad (0"
                                                 " para volver al menu): ")
                        ...
                        try:
                            ...
                            print("Cantidad invalida ingresada. Recuerda solo"
                                  " ingresar el numero de lo pedido.")
                        else:
                            pago = d(divisa_venta)
                            balance_real = usuario_actual.balance_currencies[
                                mercado_actual.divisa_compraventa]
                            # balance real va a considerar las orders activas
                            # del usuario en el mercado, para que
                            # no ingrese mas dinero del que puede pagar
                            for order in self.lista_orders:  # esto se hace
                                                # para calcular el balance real
                                if (order.mercado.divisa_compraventa ==
                                        mercado_actual.divisa_compraventa and
                                            not order.ejecutada and
                                            order.usuario == usuario_actual):
                                    if isinstance(order, Ask):
                                        balance_real -= d(order.divisa_venta)
                                    elif isinstance(order, Bid):
                            ...
                            if pago > usuario_actual.balance_currencies[
                                mercado_actual.divisa_compraventa]:
                                print("No tienes el dinero suficiente para "
                                      "hacer la order")
                            elif pago > balance_real:
                                print("No puedes hacer la order porque "
                                      "tienes mas dinero ingresado en otras.")
                            elif pago < 0 or d(moneda_de_cambio) <= 0:
                                print("Debes ingresar un numero positivo.")
                            else:
                                break
                    if volver:
                        break
                    nuevo_ask = usuario_actual.ingresar_ask(usuario_actual,
                                                            mercado_actual,
                                                            divisa_venta,
                                                            moneda_de_cambio)
                    maximo = self.maximo_id()
                ...
                self.consulta_orders_propias.append\
                    ({"username" : usuario_actual.username,
                      "Hora" : strftime("%H:%M")})
                self.estado_orders(usuario_actual)
            elif opcion == "16":
                while True:
                    id_eliminar = input("Ingresa el ID de la order que quieres"
                                        " eliminar (tiene que ser tuya!): ")
                    try:  # manejo de errores
                        int(id_eliminar)
                    except:
                        print("ID no valido")
                    else:
                        if int(id_eliminar) not in range(1,
                                                         int(self.maximo_id())
                                                                 + 1):
                ...
                usuario_actual.desplegar_balance_historico(self.lista_orders)
            input("Presiona enter para continuar  ")

    def fecha_limite(self, fecha_indicada, usuario_actual):  # para determinar
        ...
        try:
            ...
            if fecha_indicada > strftime("%Y-%m-%d") or mes_indicado > 12 \
                    or dia_indicado > 31:
                print("Fecha invalida")  # manejo de errores
                return False
            week_ago = str(DT.date.today() - DT.timedelta(days=7))
            if isinstance(usuario_actual, Investor):  # investor no
                                                    # tienen restriccion
                return True
    ...
    def upgrade(self, usuario_actual):
        ...
        if usuario_actual.balance_currencies["DCC"] < d("300000"):
            print("No tienes dinero suficiente para hacer un upgrade a "
                  "Investor.")
            return usuario_actual
        while True:  # manejo de errores
            seguro = input("Estas seguro de que quieres pasar a ser Investor "
                           "(si/no): ")
            if seguro == "si":
                nuevo_usuario = Investor(usuario_actual.username,
                                         usuario_actual.nombre,
                                         usuario_actual.apellido,
                                         usuario_actual.nacimiento)
                nuevo_usuario.balance_currencies = (usuario_actual.
                                                    balance_currencies)
    ...
    def desplegar_menu(self, usuario_actual):
        ...
        if isinstance(usuario_actual, Trader):
            ...
            if isinstance(usuario_actual, Investor):
                print("17:  Ver balance historico")
        while invalido:
            ...
            try:
                ...
                if int(opcion) not in range(1, 18):
                    pass
                else:
                    if isinstance(usuario_actual, Underaged):
                        if int(opcion) in range(1, 10):
                            invalido = False
                    elif isinstance(usuario_actual, Trader) and not \
                            isinstance(usuario_actual, Investor):
                        if int(opcion) in range(1, 17):
                            invalido = False
                    else:
                        invalido = False
    ...
    def se_hizo_despues(self, order_realizada, otra_order):  # se usa para ver
                                    # si la order realizada se hizo despues (T)
                                    # o antes (F) que la otra order
        if int(order_realizada.año) > int(otra_order.año):
            return True
    ...
    def determinar_match(self, usuario_actual, order_realizada):  # *
        ...
        asks = []  # * : se ejecuta siempre despues de realizar un bid o ask
        bids = []
        for order in self.lista_orders:
            if (order.mercado == mercado and order != order_realizada and
                        order.ejecutada == False and
                        order.usuario != usuario_actual):
                ...
                if isinstance(order, Ask) and valido:
                    ...
                    bids.append(order)
        still_left = True  # variable que es False cuando ya no hay mas match
                            # posible con la order realizada


        if isinstance(order_realizada, Ask):  # en caso de que se ponga un Ask
            opciones_bids = []  # bids que le convienen a la compra
            for bid in bids:
                if d(bid.moneda_de_cambio) >= \
                        d(order_realizada.moneda_de_cambio):
                    opciones_bids.append(bid)
            if len(opciones_bids) == 0:  # no hay match
                return
            opciones_bids.sort(key=lambda elem: float(elem.moneda_de_cambio),
                               reverse=True)
            mejores_opciones = [opciones_bids.pop(0)]
            for bid in opciones_bids:
                if bid.moneda_de_cambio == (mejores_opciones[0].
                                                    moneda_de_cambio):
            ...
            if len(mejores_opciones) == 1:  # prioridad mejor precio
                ...
                if len(investors) == 1:
                    ...
                    investors.sort(key=lambda elem: (int(elem.año),
                                                     int(elem.mes),
                                                     int(elem.dia)))
                                                    # prioridad tiempo
                    mejor_opcion = investors[0]
                else:
                    mejores_opciones.sort(key=lambda elem: (int(elem.año),
                                                            int(elem.mes),
                                                            int(elem.dia)))
                    mejor_opcion = mejores_opciones[0]  # prioridad tiempo

            tasa1 = mercado.determinar_tasa(order_realizada.usuario)
            tasa2 = mercado.determinar_tasa(mejor_opcion.usuario)
            if d(order_realizada.divisa_venta) <= \
                    d(mejor_opcion.divisa_compra):
                still_left = False
                usuario_actual.balance_currencies[mercado.divisa_compraventa] \
                    -= d(order_realizada.divisa_venta)
                usuario_actual.balance_currencies[mercado.moneda_de_cambio] \
                    += (d(order_realizada.divisa_venta) *
                        d(mejor_opcion.moneda_de_cambio) *
                        tasa1)
                mejor_opcion.usuario.balance_currencies[
                    mercado.divisa_compraventa] += (d(order_realizada.
                                                      divisa_venta) * tasa2)
                mejor_opcion.usuario.balance_currencies[
                    mercado.moneda_de_cambio] -= (d(order_realizada.
                                                    divisa_venta) *
                                                  d(mejor_opcion.
                                                    moneda_de_cambio))
                order_realizada.moneda_de_cambio = (mejor_opcion.
                                                    moneda_de_cambio)
                ...
                resto = d(mejor_opcion.divisa_compra) - d(order_realizada.
                                                          divisa_venta)
                if resto != 0:
                    new_bid = Bid(mejor_opcion.usuario, mercado, str(resto),
                                  mejor_opcion.moneda_de_cambio,
                                  mejor_opcion.tiempo)
                    maximo = self.maximo_id()
                    ...
                    mercado.orders.append(new_bid)  # quitamos el resto
                    mejor_opcion.divisa_compra = d(mejor_opcion.divisa_compra)\
                                                 - d(resto)

            elif d(order_realizada.divisa_venta) > \
                    d(mejor_opcion.divisa_compra):
                usuario_actual.balance_currencies[
                    mercado.divisa_compraventa] -= d(mejor_opcion.
                                                     divisa_compra)
                usuario_actual.balance_currencies[
                    mercado.moneda_de_cambio] += (d(mejor_opcion.divisa_compra)
                                                  * d(mejor_opcion.
                                                  moneda_de_cambio) *
                                                  tasa1)
                mejor_opcion.usuario.balance_currencies[
                ...
                mejor_opcion.usuario.balance_currencies[
                    mercado.moneda_de_cambio] -= (d(mejor_opcion.divisa_compra)
                                                  * d(mejor_opcion.
                                                     moneda_de_cambio))
                resto = d(order_realizada.divisa_venta) - d(mejor_opcion.
                                                            divisa_compra)
                mercado.asks_activos.remove(order_realizada)
                mercado.bids_activos.remove(mejor_opcion)
                order_realizada.ejecutada = True
                mejor_opcion.ejecutada = True
                new_ask = Ask(order_realizada.usuario, mercado, str(resto),
                              order_realizada.moneda_de_cambio,
                ...
                order_realizada.divisa_venta = d(order_realizada.divisa_venta)\
                                               - d(resto)
                order_realizada.moneda_de_cambio = (mejor_opcion.
                                                    moneda_de_cambio)
            if still_left:
            ...
            new_match = Match(order_realizada, mejor_opcion,
                              order_realizada.tiempo)
            self.realiza_match.append\
                ({"ask_id": new_match.ask.id,
                  "bid_id": new_match.bid.id,
                  "amount": new_match.ask.divisa_venta,
                  "price": new_match.bid.moneda_de_cambio,
                  "Hora": strftime("%H:%M"),
                  "mercado": new_match.ask.mercado.ticker})
            self.lista_match.append(new_match)
            return
        elif isinstance(order_realizada, Bid):  # en caso de que se ponga Bid
            opciones_asks = []  # asks que le convienen a la compra
            for ask in asks:
                if d(ask.moneda_de_cambio) <= \
                        d(order_realizada.moneda_de_cambio):
                    opciones_asks.append(ask)
            if len(opciones_asks) == 0:  # no hay match
                return
            opciones_asks.sort(key=lambda elem: float(elem.moneda_de_cambio),
                               reverse = False)
            mejores_opciones = [opciones_asks.pop(0)]
            for ask in opciones_asks:
                if ask.moneda_de_cambio == \
                        mejores_opciones[0].moneda_de_cambio:
            ...
            tasa1 = mercado.determinar_tasa(mejor_opcion.usuario)
            tasa2 = mercado.determinar_tasa(order_realizada.usuario)
            if d(order_realizada.divisa_compra) <= d(mejor_opcion.
                                                             divisa_venta):
                still_left = False
                usuario_actual.balance_currencies[
                    divisa_compraventa] += (d(order_realizada.divisa_compra) *
                                            tasa2)
                usuario_actual.balance_currencies[
                    moneda_de_cambio] -= (d(order_realizada.divisa_compra)
                                          * d(mejor_opcion.moneda_de_cambio))
                mejor_opcion.usuario.balance_currencies[
                    moneda_de_cambio] += (d(order_realizada.divisa_compra)
                                          * d(mejor_opcion.moneda_de_cambio) *
                ...
                order_realizada.ejecutada = True
                mejor_opcion.ejecutada = True
                resto = d(mejor_opcion.divisa_venta) - d(order_realizada.
                                                         divisa_compra)
                if resto != 0:
                    new_ask = Ask(mejor_opcion.usuario, mercado, str(resto),
                                  mejor_opcion.moneda_de_cambio,
                                  mejor_opcion.tiempo)
                    maximo = self.maximo_id()
                    new_ask.id = str(maximo + 1)
                    ...
                    mercado.orders.append(new_ask)
                    mercado.asks_activos.append(new_ask)
                    mercado.asks.append(new_ask)  # quitamos el resto
                    mejor_opcion.divisa_venta = d(mejor_opcion.divisa_venta) -\
                                                d(resto)
            elif d(order_realizada.divisa_compra) > d(mejor_opcion.
                                                              divisa_venta):
                usuario_actual.balance_currencies[
                    divisa_compraventa] += (d(mejor_opcion.divisa_venta) *
                                            tasa2)
                usuario_actual.balance_currencies[
                    moneda_de_cambio] -= (d(mejor_opcion.divisa_venta) *
                                          d(mejor_opcion.moneda_de_cambio))
                mejor_opcion.usuario.balance_currencies[
                    divisa_compraventa] -= d(mejor_opcion.divisa_venta)
                mejor_opcion.usuario.balance_currencies[
                    moneda_de_cambio] += (d(mejor_opcion.divisa_venta) *
                                          d(mejor_opcion.moneda_de_cambio) *
                                          tasa1)
                resto = d(order_realizada.divisa_compra) - \
                        d(mejor_opcion.divisa_venta)
                mercado.asks_activos.remove(mejor_opcion)
                mercado.bids_activos.remove(order_realizada)
                order_realizada.ejecutada = True
                mejor_opcion.ejecutada = True
                new_bid = Bid(order_realizada.usuario, mercado, str(resto),
                              order_realizada.moneda_de_cambio,
                              order_realizada.tiempo)
                maximo = self.maximo_id()
                new_bid.id = str(maximo + 1)
                ...
                order_realizada.divisa_compra = d(order_realizada.
                                                  divisa_compra) - d(resto)
                order_realizada.moneda_de_cambio = (mejor_opcion.
                                                    moneda_de_cambio)
            ...
            mejor_opcion.match_time = strftime("%Y-%m-%d", gmtime())
            order_realizada.match_time = strftime("%Y-%m-%d", gmtime())
            new_match = Match(mejor_opcion, order_realizada,
                              order_realizada.tiempo)
            self.realiza_match.append\
                ({"ask_id": new_match.ask.id,
                  "bid_id": new_match.bid.id,
    ...
    def determinar_match_parciales_csv(self):
        lista_orders_ordenada = sorted(self.lista_orders,
                                       key=lambda elem: (int(elem.año),
                                                         int(elem.mes),
                                                         int(elem.dia)),
                                       reverse = False)
