...
class Usuario:  # de aca se heredan 3 tipos de usuarios
    ...
    @nacimiento.setter
    def nacimiento(self, value):  # se usa para no ingresar fechas afuera de 1900 o 2017
        try:
            año = value[0:4]
            mes = value[5:7]
            dia = value[8:]
    ...
    def agregar_mercado(self, mercado):
        ...
        if mercado.divisa_compraventa not in self.balance_currencies.keys():
            self.balance_currencies.update({mercado.divisa_compraventa : d("50000")})  # se agregan los 50.000
        else:
...
class Underaged(Usuario):
    ...
    def agregar_mercado(self, mercado):  # solo agrega el mercado, no se le agrega nada al saldo porque no tiene
        self.mercados.append(mercado)
...
class Trader(Usuario):
    ...
    def generar_balance(self):  # agregar al diccionario de balances a partir de los mercados registrados
